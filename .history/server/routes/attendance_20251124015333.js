...
router.get("/", (req, res) => {
  ...
  let query = `
    SELECT a.*, s.name AS student_name, s.student_code, sub.subject_code, sub.subject_title
    FROM attendance a
  ...
  if (userId) {
    query += ` WHERE a.student_id = (SELECT id FROM students WHERE name = (SELECT name FROM students WHERE student_id = ?) LIMIT 1)`;
  }
  query += " ORDER BY a.date DESC";

  db.query(query, studentId ? [userId] : [], (err, results) => {
...
router.post("/", (req, res) => {
  ...
  if (!Number.isNaN(numericId)) {
    ...
    db.query("SELECT id FROM students WHERE student_code = ? LIMIT 1", [student_id], (e, rows) => {
